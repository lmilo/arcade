...
export class Pong extends Game {
  ...
  update(dt: number, input: Input) {
    ...
    // IA imperfecta: persigue la bola con velocidad limitada que escala con el score.
    ...
    this.aiX = Math.max(PADDLE_W / 2, Math.min(W - PADDLE_W / 2, this.aiX))
    ...
    this.paddleBounce(this.playerX, PLAYER_Y, true)
    this.paddleBounce(this.aiX, AI_Y, false)
  ...
  render(ctx: CanvasRenderingContext2D) {
    ...
    ctx.beginPath()
    ctx.roundRect(this.aiX - PADDLE_W / 2, AI_Y - PADDLE_H / 2, PADDLE_W, PADDLE_H, 6)
    ctx.fill()
  ...
  private speed() {
    return Math.min(460, 260 + this.score * 7)
  }
  ...
  private launch() {
    ...
    this.ballVY = speed * Math.cos(angle) // hacia abajo, al jugador
  }

  private paddleBounce(px: number, py: number, isPlayer: boolean) {
    const movingToward = isPlayer ? this.ballVY > 0 : this.ballVY < 0
    if (!movingToward) return
    const near = isPlayer ? this.ballY + BALL_R >= py - PADDLE_H / 2 && this.ballY < py : this.ballY - BALL_R <= py + PADDLE_H / 2 && this.ballY > py
    if (!near || Math.abs(this.ballX - px) > PADDLE_W / 2) return
    const offset = Math.max(-1, Math.min(1, (this.ballX - px) / (PADDLE_W / 2)))
    const speed = this.speed()
